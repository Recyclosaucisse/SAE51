...
// APPEUI ligne 71 : 0C 7E 45 01 02 03 03 46
static const u1_t PROGMEM APPEUI[8]={ 0x0C, 0x7E, 0x45, 0x01, 0x02, 0x03, 0x03, 0x46  }; // 8 octets  
void os_getArtEui (u1_t* buf) { memcpy_P(buf, APPEUI, 8);}

// DEVEUI -> APPEUI in little Indian
static const u1_t PROGMEM DEVEUI[8]={ 0x46, 0x03, 0x03, 0x02, 0x01, 0x45, 0x7E, 0x0C }; 
void os_getDevEui (u1_t* buf) { memcpy_P(buf, DEVEUI, 8);}

//APPKEY Ligne 69 : 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 03 45
static const u1_t PROGMEM APPKEY[16] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x03, 0x46 }; //16 octets
void os_getDevKey (u1_t* buf) {  memcpy_P(buf, APPKEY, 16);}

static uint8_t mydata[] = "init"; 
static osjob_t sendjob;
const unsigned TX_INTERVAL = 60; // Waiting time (in seconds) between 2 LoRaWAN transmissions
int presence;
...
void do_send(osjob_t* j) // Foction used to send LoRaWAN packets
{
  ...
  else 
  {
    ...
    }//Returns the battery remaining percentage. 0/25/50/75/100, returns -1 if it can not communicate with the controller
...
void setup() 
{
  ...
  {
    timeClient.forceUpdate(); // Synchronization of the clock using a NTP server
  }
  M5.Lcd.println("Clock settings synchronized");
  pinMode(21, INPUT); // Set the PINs of the PIR captor
