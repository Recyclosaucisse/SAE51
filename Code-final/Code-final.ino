...
// Name:M5Stack01-19 : BA 03 03 02 01 45 7E 0C
static const u1_t PROGMEM APPEUI[8]={ 0xBA, 0x03, 0x03, 0x02, 0x01, 0x45, 0x7E, 0x0C }; // 8 octets  
void os_getArtEui (u1_t* buf) { memcpy_P(buf, APPEUI, 8);}

// MAC address of the M5Stack : 0C 7E 45 01 02 03 03 13
static const u1_t PROGMEM DEVEUI[8]={ 0x0C, 0x7E, 0x45, 0x01, 0x02, 0x03, 0x03, 0xBA }; 
void os_getDevEui (u1_t* buf) { memcpy_P(buf, DEVEUI, 8);}

// Application Key : 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 03 13 -> Used to cypher the communication
static const u1_t PROGMEM APPKEY[16] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x03, 0x13 }; //16 octets
void os_getDevKey (u1_t* buf) {  memcpy_P(buf, APPKEY, 16);}

static osjob_t sendjob; //Variable necessary for proper functioning of the LMIC library
static uint8_t mydata[] = "init"; //initializing mydata variable used in do_send function (table)
...
void do_send(osjob_t* j) //Allows to send data in LoRaWAN
{
  static uint32_t sqn = 0; //Sequence number
  if (LMIC.opmode & OP_TXRXPEND) //If there is a current TX/RX job running
  {
    M5.Lcd.println("OP_TXRXPEND, not sending");
  }
  else //If there isn't a current TX/RX job running
  {
    if (digitalRead(16)==1)// If mouvement detected
    {
      M5.Lcd.println("mouvement detected");
      int payload_true = 1;
      sprintf((char*)mydata,"{\"SAE51\test\capteur1\":%d}", payload_true); //JSON format 
    }
    else
    {
      M5.Lcd.println("no mouvement detected");
      int payload_false = 0;
      sprintf((char*)mydata,"{\"Presence\":%d}", payload_false); //JSON format 
    }
    LMIC_setTxData2(1, mydata, strlen((char*)mydata), 0); //Sending data without ACK (beacause there is a 0 at the end)
    sqn++; //Sequence number incremented
    //char char_sqn = sqn; //Converts sqn variable into a character so we can display it after
...
void setup() { //When starting the device
  M5.begin(); 
  M5.Power.begin();
  M5.Lcd.begin();
  ...
  LoRa.begin(868E6); // frequency in Hz
  if (!LoRa.begin(868E6)) //If start of LoRa module fails
  {
...
void loop() //Loops everytime while the device is ON
{
  ...
  do_send(&sendjob); //calling the fonction do_send
  delay(TX_INTERVAL*1000); //waiting for 10s, see TX_INTERVAL at the beginning of the code
}